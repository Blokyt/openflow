...
def test_budget_view_matches_entity_balance_for_period(client_and_db):
    """opening_balance + realized_total from /view == compute_entity_balance_for_period closing."""
    ...
    # opening_balance from /view is compute_entity_balance at 2025-08-31
    opening = club["opening_balance"]

    conn = sqlite3.connect(str(db_path))
    conn.row_factory = sqlite3.Row
    try:
        direct = compute_entity_balance_for_period(conn, e["id"], "2025-09-01", "2026-08-31", opening=opening)
    finally:
        conn.close()
...
def test_view_ignores_transactions_outside_period(client):
    ...
    # After period (open mandate: end = today, so 2026-10-15 is after today 2026-04-25 → actually it IS after)
    # Use a date after today + far future
    client.post("/api/transactions/", json={
        "date": "2029-10-15", "label": "post", "amount": 1000.0,
        "from_entity_id": ext["id"], "to_entity_id": e["id"],
    })

    view = client.get(f"/api/budget/view?fiscal_year_id={fy['id']}").json()
    club = next(x for x in view["entities"] if x["entity_id"] == e["id"])
    # pre-period tx affects opening, not realized_total; far-future tx is out of range
    # opening = 1000 (the pre-period tx), realized = 0
