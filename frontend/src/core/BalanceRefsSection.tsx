  ...
  if (loading) {
    return (
      <section id="balances" className="mb-8">
        ...
        <div className="bg-bg-card border border-border rounded-2xl p-5 flex items-center justify-center h-20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-[1fr_110px_130px_160px_130px_44px] gap-x-3 gap-y-2 items-center">
                ...
                <div>
                  {isRoot ? (
                    <select
                      ...
                      className="w-full bg-[#0a0a0a] border border-border-hover rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none focus:border-accent-sand cursor-pointer"
                    >
                ...
                <div className="flex flex-col gap-1">
                  <input
                    ...
                    className="w-full bg-[#0a0a0a] border border-border-hover rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none focus:border-accent-sand [color-scheme:dark]"
                  />
                </div>
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      placeholder="0"
                      className="w-full bg-[#0a0a0a] border border-border-hover rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none focus:border-accent-sand"
                    />
                    <span className="text-xs text-[#555] flex-shrink-0">€</span>
                  </div>
                  {r.mode === "aggregate" && (
                    <span className="text-xs text-accent-sand/70 leading-tight">solde agrégé bancaire</span>
                  )}
                  {hint && (
                    <span className="text-[10px] text-alert/80 leading-tight">
                ...
                <div className="text-sm text-right">
                  {r.currentBalance !== null ? (
                    <span className={r.currentBalance >= 0 ? "text-success" : "text-alert"}>
                ...
                <div className="flex items-center justify-end h-8">
                  {r.savedOk ? (
                    ...
                    <span className="text-[10px] text-alert leading-tight text-right max-w-[80px]">{r.error}</span>
                  ) : dirty ? (
                    <button
                      onClick={() => handleSave(entity)}
                      disabled={r.saving}
                      className="p-1.5 rounded-lg bg-accent-sand/10 border border-accent-sand/30 text-accent-sand hover:bg-accent-sand/20 transition-colors disabled:opacity-50"
                      title="Enregistrer"
                    >
                      {r.saving ? (
                        <PageLoader fullScreen={false} />
